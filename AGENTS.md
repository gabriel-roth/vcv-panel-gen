...
Accurate crop: `patch`/`live` never compute the module's on-screen rectangle from Rack's scroll/zoom/Retina state (the arithmetic that used to drift). They render the module's default state as a template and **locate it inside the window capture by normalized cross-correlation over gradient magnitude** (`shotmatch.py`, FFT-based), then crop the matched box. Matching on gradients (edges), not raw pixels, makes it invariant to the light-vs-dark panel theme (the light template still matches a dark on-screen panel) and shrugs off cables and lit LEDs. A match below `--min-score` (default 0.5) is a hard `ERROR:`, never a silently-wrong crop.
